...
class PreSale extends Base
{
    ...
    public function info(ActivityLogic $logic)
    {
        ...
        $this->param['g_field'] = 'goods_id, key, key_name, price, original_price, pre_sale_price, deposit, final_payment_time, sales_price, cost_price, profits_percent, store_count, spec_img, limited_quantity';
        $validate               = new activityValidate();
        $validate_result        = $validate->scene('info')->check($this->param);
        if (!$validate_result) {
            return JsonUtils::fail($validate->getError());
        }
        $info = $logic->getInfo($this->param);
        return $info;
    }
