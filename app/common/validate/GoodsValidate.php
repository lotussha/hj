<?php
/**
 * Created by PhpStorm.
 * User: Jomlz
 * Date: 2020/8/4
 * Time: 19:56
 */

namespace app\common\validate;

use think\facade\Db;
use think\Validate;

class GoodsValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'goods_id' => 'require|checkGoodsId',
//        'goods_name' => 'require|min:3|max:150|unique:goods',
        'goods_name' => 'require|min:3|max:150',
        'cat_id' => 'number|gt:0|isCatHas',
        'goods_sn' => 'unique:goods|max:20',
//        'shop_price' => ['require', 'regex' => '([1-9]\d*(\.\d*[1-9])?)|(0\.\d*[1-9])'],
        'shop_price' => 'require|regex:\d{1,10}(\.\d{1,2})?$|checkShopPrice',
        'market_price' => 'require|regex:\d{1,10}(\.\d{1,2})?$|checkMarketPrice',
        'cost_price' => 'require|regex:\d{1,10}(\.\d{1,2})?$',
        'weight' => 'regex:\d{1,10}(\.\d{1,2})?$',
        'volume' => 'regex:\d{1,10}(\.\d{1,2})?$',
        'give_integral' => 'regex:^\d+$',
        'is_virtual' => 'checkVirtualIndate',
        'exchange_integral' => 'checkExchangeIntegral',
        'is_free_shipping' => 'require|checkShipping',
        'goods_type' => 'require|number|isGoodsType',
        'warehouse_id' => 'number|isWarehouse',
        'one_distribution_price|一级分销金额' => 'require|regex:\d{1,10}(\.\d{1,2})?$',
        'two_distribution_price|二级分销金额' =>  'require|regex:\d{1,10}(\.\d{1,2})?$',
        'let_profits_price|让利金额' =>  'require|regex:\d{1,10}(\.\d{1,2})?$',
        'replace_sell_price|代售金额' =>  'regex:\d{1,10}(\.\d{1,2})?$',
        'key|规格组合值'=> 'require',
        'field|字段名'=> 'require',
        'value|字段值'=> 'require',
        'goods_common|商品规格'=> 'require|checkCommon',
//        'commission' => 'checkCommission',
//        'ladder_amount' => 'checkLadderAmount',
//        'ladder_price' => 'checkLadderPrice',
//        'item' => 'checkItems',
    ];
    //错误信息
    protected $message = [
        'goods_name.require' => '商品名称必填',
        'goods_name.min' => '名称长度至少3个字符',
        'goods_name.max' => '名称长度至多50个汉字',
        'goods_name.unique' => '商品名称重复',
        'cat_id.number' => '商品分类必须填写',
        'cat_id.gt' => '商品分类必须选择',
        'cat_id.isCatHas' => '商品分类不存在',
        'goods_sn.unique' => '商品货号重复',
        'goods_sn.max' => '商品货号超过长度限制',
        'goods_num.checkGoodsNum' => '抢购数量不能大于库存数量',
        'shop_price.require' => '本店售价必填',
        'cost_price.require' => '成本价必填',
        'shop_price.regex' => '本店售价格式不对',
        'cost_price.regex' => '成本价格式不对',
        'market_price.require' => '市场价格必填',
        'market_price.regex' => '市场价格式不对',
        'market_price.checkMarketPrice' => '市场价不得小于本店价',
        'shop_price.checkShopPrice' => '门店价不得小于成本价',
        'weight.regex' => '重量格式不对',
        'volume.regex' => '体积格式不对',
        'give_integral.regex' => '赠送积分必须是正整数',
        'exchange_integral.checkExchangeIntegral' => '积分抵扣金额不能超过商品总额',
        'is_virtual.checkVirtualIndate' => '虚拟商品有效期不得小于当前时间',
        'is_free_shipping.require' => '请选择商品是否包邮',
        'goods_type.isGoodsType' => '商品模型不存在',
        'warehouse_id.isWarehouse' => '仓库不存在',
    ];

    protected $scene = [
        'add'   => ['goods_name','cat_id','shop_price','market_price','cost_price','weight','volume','is_free_shipping','goods_type','warehouse_id','one_distribution_price','two_distribution_price','let_profits_price','goods_common'],
        'edit'   => ['goods_id','cat_id','shop_price','market_price','cost_price','weight','volume','is_free_shipping','goods_type','warehouse_id','one_distribution_price','two_distribution_price','let_profits_price','goods_common'],
        'del'   => ['goods_id'],
    ];

    public function sceneSpec()
    {
        return $this->only(['goods_id','key'])->remove('goods_id','checkGoodsId');
    }
    public function sceneField()
    {
        return $this->only(['goods_id','field','value'])->remove('goods_id','checkGoodsId');
    }

    public function sceneInfo()
    {
        return $this->only(['goods_id'])->remove('goods_id','checkGoodsId');
    }
//    public function sceneDel()
//    {
//        return $this->only(['goods_id'])->remove('goods_id','checkGoodsId');
//    }

    /**
     * 判断分类
     */
    protected function isCatHas($value,$rule,$data)
    {
        $cat = Db::name('goods_category')->where(['id'=>$data['cat_id']])->find();
        if(!$cat){
            return '分类不存在';
        }else{
            return true;
        }
    }

    /**
     * 判断规格参数
     */
    public function checkCommon($value,$rule,$data)
    {
       $goods_common = json_decode($data['goods_common'],true);
       foreach ($goods_common as $k=>$v){
         if (empty($k) || empty($v['price']) || empty($v['key_name']) || empty($v['market_price']) || empty($v['cost_price']) || empty($v['wholesale_num']) || empty($v['wholesale_price']) || empty($v['store_count']) || empty($v['original_price'])){
             return '规格参数需要填完整';
         }
       }
       return true;
    }

    /**
     * 判断仓库
     * User: Jomlz
     */
    protected function isWarehouse($value,$rule,$data)
    {
        if ($data['warehouse_id'] > 0){
            $cat = Db::name('admin_users')->where(['id'=>$data['warehouse_id'],'identity'=>5])->find();
            if(!$cat){
                return '仓库不存在';
            }
            return true;
        }else{
            return true;
        }

    }

    /**
     * 检测模型
     * User: Jomlz
     */
    protected function isGoodsType($value,$rule,$data)
    {
        $cat = Db::name('goods_type')->where(['id'=>$data['goods_type']])->find();
        if(!$cat){
            return '模型不存在';
        }else{
            return true;
        }
    }

    /**
     * 检测运费模板
     * User: Jomlz
     */
    protected function checkShipping($value,$rule,$data){
        if($value == 0 && empty($data['template_id'])){
            return '请选择运费模板';
        }else{
            return true;
        }
    }

    protected function  checkMarketPrice($value,$rule,$data){
        if($value < $data['shop_price']){
            return false;
        }else{
            return true;
        }
    }

    protected function  checkShopPrice($value,$rule,$data){
        if($value < $data['cost_price']){
            return false;
        }else{
            return true;
        }
    }

    //检查阶梯价格中的库存
    protected function checkLadderAmount($value, $rule, $data)
    {
        if(min($value) != '' && min($value) <= 0){
            return  '您没有输入有效的库存阶梯！';
        }
        return true;
    }

    //检查阶梯价格中的价格
    protected function checkLadderPrice($value, $rule, $data)
    {
        if(max($value) >= $data['shop_price']){
            return '价格阶梯最大金额不能大于商品原价！';
        }
        if(min($value) != '' && min($value) <= 0){
            return '您没有输入有效的价格阶梯！';
        }
        return true;
    }

    protected function checkItems($value, $rule, $data)
    {
        foreach($data['item'] as $itemKey => $item){
            if($item['price'] > $item['market_price']){
                return '规格市场价不得小于购买价';
            }
        }
        return true;
    }

    //检查是否有商品规格参加活动，若有则不能编辑商品
    protected function checkGoodsId($value)
    {
        $spec_goods_price = Db::name('goods_spec_price')->where('goods_id', $value)->where('prom_type', '>', 0)->find();
        if ($spec_goods_price) {
            return '商品规格为' . $spec_goods_price['key_name'] . '正在参与活动，不能操作该商品信息';
        }
        $goods = Db::name('goods')->where('goods_id', $value)->find();
        if ($goods['prom_type'] > 0) {
            return '该商品正在参与活动，不能编辑该商品信息';
        }
        return true;
    }

    //检查虚拟商品有效时间
    protected function  checkVirtualIndate($value, $rule, $data)
    {
        $virtualIndate = strtotime($data['virtual_indate']);
        if ($value == 1 && time() > $virtualIndate) {
            return false;
        } else {
            return true;
        }
    }

    protected function checkCommission($value, $rule, $data)
    {
        if ($value >= $data['cost_price']) {
            return '商品分销的分成金额不能大于等于商品成本价格金额';
        } else {
            return true;
        }
    }

    // 检查积分兑换
    protected function checkExchangeIntegral($value, $rule, $data)
    {
        if ($value > 0) {
            $goods = Db::name('goods')->where('goods_id', $data['goods_id'])->find();
            if (!empty($goods)) {
                if ($goods['prom_type'] > 0) {
                    return '该商品参与了其他活动。设置兑换积分无效，请设置为零';
                }
            }
        }
        $point_rate_value = cache('shopping.point_rate');
        if ($data['item']) {
            $count = count($data['item']);
            $item_arr = array_values($data['item']);
            $minPrice = $item_arr[0]['price'];
            for ($i = 0; $i < ($count - 1); $i++) {
                if ($item_arr[$i + 1]['price'] < $minPrice) {
                    $minPrice = $item_arr[$i + 1]['price'];
                }
            }
            $goods_price = $minPrice;
        } else {
            $goods_price = $data['shop_price'];
        }

        $point_rate_value = empty($point_rate_value) ? 0 : $point_rate_value;
        if ($value > ($goods_price * $point_rate_value)) {
            return '积分抵扣金额不能超过商品总额';
        } else {
            return true;
        }
    }

}